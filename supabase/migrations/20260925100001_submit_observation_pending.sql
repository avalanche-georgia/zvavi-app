-- Public observations now go through moderation: new submissions land as
-- `pending` instead of `published`, and only appear publicly once an admin
-- approves them. Same signature as 20260923120000_submit_observation_add_photo_keys.sql,
...
BEGIN
  ...
  INSERT INTO recent_avalanches (
    ...
    'external', 'pending', p_quantity, p_slab_depth, p_width, nullif(p_photo_keys, '{}')
  )
